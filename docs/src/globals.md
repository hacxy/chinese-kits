**chinese-kits** • [Readme](README.md) \| API

***

# chinese-kits

## 类型别名

- [DateType](type-aliases/DateType.md)
- [LunarDate](type-aliases/LunarDate.md)

## 农历

- [getDateDiff](functions/getDateDiff.md)
- [getLunarDate](functions/getLunarDate.md)

## 字符串

- [isContainsChinese](functions/isContainsChinese.md)
- [isOnlyChinese](functions/isOnlyChinese.md)

## 手机号

- [isChinesePhoneNumber](functions/isChinesePhoneNumber.md)

## 省份

- [getAllCityAlias](functions/getAllCityAlias.md)
- [getCityAlias](functions/getCityAlias.md)

## 节假日

- [isHolidays](functions/isHolidays.md)
- [isInLieuDays](functions/isInLieuDays.md)
- [isWorkdays](functions/isWorkdays.md)
